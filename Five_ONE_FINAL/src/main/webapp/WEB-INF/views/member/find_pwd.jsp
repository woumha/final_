...
function findPwd(){
		...
		$.ajax({
			url : '<%=request.getContextPath()%>/find_pwd_ok.do',
			data : {
				id : input_id,
				email : input_email
			},
			success : function(data){
				...
				$("#find-card").css("height", "200px");
				$("#find-card").html("<span class='find-txt'></span>");
				
				if (data == "undefined"){
					$("#find-card span").text("이메일과 일치하는 아이디가 없습니다.");
				}else if (data == "social"){
					$("#find-card span").text("소셜 로그인을 통해 가입된 아이디입니다.");
				}else if (data == "mail"){
					$("#find-card").css("flex-direction", "column");
					$("#find-card span").text("임시 비밀번호가 발송되었습니다.");
					$("#find-card").append("<span class='find-txt'></span>");
					$("#find-card span:nth-child(2)").text("이메일을 확인해주세요.");
				}
			},
			error : function(){
					...
					<div id="find-card">
						<div class="input-wrap">
							<div class="label-input" >
								...
								<input name="member_id" class="member-input form-input" id="input-id">
						</div>
						<div class="input-wrap">
							<div class="label-input" >
								<label for="input-id" id="input-id-check"></label>
						...
						<div class="input-wrap">
							<input type="button" value="비밀번호 찾기" class="form-btn" onclick="findPwd()">
						</div>
					</div>
					<br>
					<div id="social-card">
